// Adds the "Mode Privat" columns (Guest.deviceIds, Guest.maxDevices,
// WeddingConfig.privateMode) WITHOUT dropping data.
// (Never use db:push-turso for this — that script recreates tables and wipes rows.)
// Applies to both the local dev.db and the remote Turso DB.
const path = require("path");
require("dotenv").config();

const ALTERS = [
  `ALTER TABLE "Guest" ADD COLUMN "deviceIds" TEXT NOT NULL DEFAULT '[]'`,
  `ALTER TABLE "Guest" ADD COLUMN "maxDevices" INTEGER NOT NULL DEFAULT 2`,
  `ALTER TABLE "WeddingConfig" ADD COLUMN "privateMode" BOOLEAN NOT NULL DEFAULT false`,
];
